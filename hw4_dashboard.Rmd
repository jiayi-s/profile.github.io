---
title: "HW4 -- New York NOAA weather data, 1980-2010"
output: 
...
### Average temperature in August (1980-2010) among 10 randomly selected stations.
...
ny_noaa %>% 
  ...
  layout(yaxis = list(title = 'Average Temperature each year'))
```

Column {data-width=400}
-----------------------------------------------------------------------

### Tmax VS Tmin of station "USC00305113" (1981-2010)
...
ny_noaa %>% 
  ...
  plot_ly(x = ~tmin, y = ~tmax, color = ~prcp, type = "scatter", mode = "markers",
          alpha = 0.4) %>% 
  layout(xaxis = list(title = 'Min Temp')) %>% 
  layout(yaxis = list(title = 'Max Temp'))
